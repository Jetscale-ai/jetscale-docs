...
infrastructure cloud et les livre sous forme de code Terraform prêt pour la
production. Ce guide explique le workflow de la plateforme Jetscale - ce qui se
passe lorsque vous connectez votre compte cloud et comment vous obtenez des
économies concrètes.
...
2. **Examiner** → Parcourir les recommandations générées par l'IA avec
   estimations d'économies
3. **Déployer** → Obtenir le code Terraform via pull request, fusionner et
   économiser
...
- Accorder à Jetscale un accès en lecture seule à votre environnement AWS ou
  Azure
- Aucune permission d'écriture - Jetscale ne modifie jamais directement votre
  infrastructure
...
- **Aucune information d'identification stockée** : Tout utilise des jetons
  temporaires et à portée limitée
- **Permissions en lecture seule** : Jetscale ne peut que consulter les
  métadonnées de ressources et les métriques
...
Contrairement aux outils de coût génériques qui vous alertent simplement d'une
faible utilisation CPU, l'IA de Jetscale comprend les nuances de chaque service
cloud :
...
- **Évaluation de l'impact sur les performances** : Zéro temps d'arrêt ou
  ajustements mineurs notés
- **Niveau de risque d'implémentation** : Faible, Moyen ou Élevé avec risques
  spécifiques
...
- Filtrer par critères (ex. "toutes les recommandations à faible risque >
  100$/mois")
...
- La PR inclut documentation complète, analyse de coûts et liste de vérification
  de tests
...
- **Tendances d'utilisation des ressources** : Repérer de nouvelles opportunités
  d'optimisation
...
- Jetscale n'a besoin que d'un accès en lecture aux configurations de ressources
  et métriques
...
- [Analyse approfondie de l'IA](ai-analysis.md) - Comment notre IA valide la
  sécurité
