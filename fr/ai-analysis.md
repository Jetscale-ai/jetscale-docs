...
> Comment l'IA de Jetscale génère des recommandations d'optimisation des coûts
> tout en maintenant les performances et la fiabilité

## Vue d'ensemble

Jetscale utilise une IA avancée pour analyser votre infrastructure cloud et
générer des recommandations sûres et validées. Notre IA ne se contente pas
d'identifier des économies de coûts. Elle garantit que ces économies
maintiennent les performances, la disponibilité et la fiabilité.
...
Contrairement aux outils génériques de coûts cloud qui fournissent de simples
alertes ("Votre CPU est bas, réduisez la taille !"), l'IA de Jetscale comprend
les nuances de chaque service cloud :
...
- Exigences de marge des Auto Scaling Groups pour les événements de mise à
  l'échelle
...
- Caractéristiques de performance des types de volumes selon les charges de
  travail
...
- Analyse des tendances de croissance (utilisation croissante nécessite un
  tampon plus grand)
...
L'IA de Jetscale évalue plusieurs alternatives et sélectionne l'équilibre
optimal :
...
**Ce qu'elle fait :** Détermine la taille d'instance optimale basée sur les
modèles d'utilisation réels et les caractéristiques de la charge de travail.
...
- Analyse du taux de croissance (croissance de 10% par an nécessite un tampon
  plus grand)
...
**Ce qu'elle fait :** Identifie les ressources adaptées aux engagements de 1 an
ou 3 ans pour débloquer des réductions de 40-60%.
...
**Ce qu'elle fait :** Identifie les charges de travail pouvant bénéficier de
30-40% d'économies sur les instances AWS Graviton basées ARM.

#### Facteurs IA considérés :

- Compatibilité applicative (charges de travail conteneurisées idéales)
- Profil de performance (Graviton3 excelle pour le web, bases de données, mise
  en cache)
...
**Ce qu'elle fait :** Optimise les types de volumes EBS (gp2→gp3) et ajuste les
IOPS/débit selon les besoins réels.
...
- **Impact sur les performances post-déploiement** : Surveillance de la
  dégradation
...
- Ajuster selon les préférences spécifiques à l'organisation (conservateur vs
  agressif)
...
- **Pourquoi ce changement est suggéré** : Modèles d'utilisation spécifiques et
  inefficacités
...
- Définir des politiques personnalisées (ex : "ne jamais toucher les bases de
...
- [Comment fonctionne Jetscale](how-it-works.md) - Flux de travail global de la
  plateforme
