...
> Human-readable catalog of HTTP routes exposed under **`/api/v2`**, plus OAuth
> on **`/oauth/*`**. For request and response schemas, use the interactive
> OpenAPI UI on a running server.

This page groups every route by **module** (router prefix). Most handlers return
the V2 JSON envelope **`BaseApiResponse`** (`meta` + `data`). Exceptions are
...
**Authentication (typical):** Session and Bearer tokens issued by `/api/v2/auth`
flows. Individual operations require the permissions configured for your tenant
and role.
...
| `GET`   | `/api/v2/auth/role-assignments`                 | List role assignments with filters and pagination. |
...
- [API Reference](api-reference.md) — product-facing overview and future public
  API notes.
- Open **`/docs`** or **`/redoc`** on your Jetscale deployment for the
